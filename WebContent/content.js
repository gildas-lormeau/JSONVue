...
function displayError(error, loc, offset) {
  ...
  if (start == end - 1) range.setEnd(pre, start);
  else range.setEnd(pre, end);
...
function displayUI(theme, html) {
  ...
  content +=
    ...
    chrome.runtime.getURL('jsonview-core.css') +
    '">';
  ...
  toolboxElement.style.padding = '2px';
  toolboxElement.style.borderRightWidth = '1px';
  toolboxElement.style.borderBottomRightRadius = '4px';
  toolboxElement.style.marginRight = '1.25em';
...
var onmouseMove = (function() {
  ...
  return function(event) {
    ...
    if (element) {
      jsonSelector = [];
      if (hoveredLI) hoveredLI.firstChild.classList.remove('hovered');
      hoveredLI = element;
...
function load() {
  ...
  if (
    ...
    child = document.body.children.length
      ? document.body.childNodes[0]
      : document.body;
